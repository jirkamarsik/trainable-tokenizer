...
void* FeatureExtractor::operator()(void *input_p) {
	...
	for (std::vector<token_t>::iterator token = chunk_p->tokens.begin();
		...
		token->property_flags = std::vector<bool>(m_n_properties);

		for (int i = 0; i < m_regex_properties.size(); i++) {
			token->property_flags[i] = m_regex_properties[i].FullMatch(token->text);
		}

		typedef std::multimap<std::string, int>::const_iterator iter;
