...
#define CHECK_DECISION_FLAG(flag) {\
  context.push_back(make_pair(\
        offset_str + "%" #flag,\
        questioned_token.decision_flags & flag##_FLAG ? 1.0 : 0.0));\
}
   ...
   || (center_token.decision_flags & MAY_BREAK_SENTENCE_FLAG)) {
    ...
    for (int offset = -m_precontext; offset != m_postcontext + 1; offset++) {
      ...
      context.push_back(make_pair(
            ...
            center_token.n_newlines >= 0 ? 1.0 : 0.0));
      context.push_back(make_pair(
            offset_str + "%LINE_BREAK",
            center_token.n_newlines >= 1 ? 1.0 : 0.0));
      context.push_back(make_pair(
            offset_str + "%PARAGRAPH_BREAK",
            center_token.n_newlines >= 2 ? 1.0 : 0.0));
